...
tn='gcc-8.2.0'; url='http://mirrors.concertpass.com/gcc/releases/gcc-8.2.0/gcc-8.2.0.tar.xz'; 
set_source 'tar';
...
config_dest;`src_path`/configure $intermediate --with-pic --enable-targets=x86-64-linux --disable-multilib \
		--enable-gold=yes --enable-languages=c,c++,fortran,lto,objc,obj-c++ --enable-static --enable-shared \
