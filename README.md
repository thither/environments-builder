...
* Main installation targets of the Build Enviroment are: nftables, PyPy2, Apache-Hadoop and Hypertable
 ...
 * Edit the function _do_build() and add a named-source case in follow structure:
  ...
  ```
      shift;;
  ```
 * or the same format as the _do_build function's case, add a bash file in SCRIPTS_PATH directory with the filename named-source.sh (The source filenam applies first, if exists)
